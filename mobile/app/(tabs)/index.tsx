...
import { FlatList, RefreshControl, StyleSheet, Text, TouchableOpacity, View, ScrollView } from 'react-native';
...
import { darkColors, fontSizes, lightColors, spacing, shadows, borderRadius } from '@/constants/theme';
...
import type { DashboardData, SavingsPlan, Activity } from '@/types';
        ...
        <View style={[styles.header, { paddingTop: spacing.xl + insets.top }]}>
          <View style={styles.avatarSection}>
            ...
            <View style={styles.greetingSection}>
              ...
              <View style={{ width: 140, height: 14, backgroundColor: colors.lineFaint, borderRadius: 4, marginTop: 6 }} />
            </View>
          </View>
        </View>
        {/* Tab skeleton */}
      ...
      {activeTab === 'overview' && (
        <>
          ...
          {dashboard?.activities && dashboard.activities.length > 0 && (
            <Animated.View entering={FadeInUp.delay(300).duration(300)} style={[styles.activityCard, { backgroundColor: colors.surface, borderColor: colors.line }]}>
              ...
              {dashboard.activities.slice(0, 5).map((a: Activity) => {
                ...
                return (
                  <View key={a.id} style={styles.activityItem}>
                    <View style={[styles.activityDot, { backgroundColor: isMe ? colors.gold : colors.mate }]} />
                    <View style={styles.activityContent}>
                      <Text style={[styles.activityText, { color: colors.txtPrimary }]}>
                        <Text style={{ fontWeight: '600', color: isMe ? colors.gold : colors.mate }}>
                          {isMe ? '我' : a.nickname}
                        </Text>
                        {' '}{label}
                        {a.detail && <Text style={{ color: colors.txtSecondary }}> {a.detail}</Text>}
                        {a.action === 'react' && <Text style={{ color: colors.txtSecondary }}> 发送了表情</Text>}
                      </Text>
            ...
            renderItem={({ item, index }) => {
              ...
              return (
                ...
                >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      {dlInfo.label && (
                        <Text style={[
                          ...
                          <FontAwesome name={isOverdue ? 'exclamation-circle' : 'calendar'} size={10} /> {dlInfo.label}
                        </Text>
                      )}
                    </View>
                    <View
                  ...
                  {isOverdue && (
                    <View style={[styles.encouragement, { backgroundColor: 'rgba(245, 158, 11, 0.08)', borderColor: 'rgba(245, 158, 11, 0.15)' }]}>
